...
audio_file = "../resampled_audio_files/cut_sample_3.wav"
...
# Loop over filtered_hidden_states to identify step start and end points
...
for start, end in zip(step_starts, step_ends):
        if all(value <= 0.1 for value in y_smooth[start:end]):
            filtered_hidden_states[start:end] = 0
...
with open("cs_3.json", "w") as json_file:
